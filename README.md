...
> **v2.0.0 重大变更**：插件目录与包名由 `SleepData` 更名为 `HealthData`；已移除 iOS 快捷指令相关能力。请将插件文件夹重命名/替换为 `usr/plugins/HealthData`，并在后台重新启用。主题中请改用 `HealthData_Plugin::…`。
...
- 查询接口 `health-api.php`；主题方法 `getTodayHealthData` / `getLatestHealthData` / `getTodaySleepData` / `getLatestSleepData`
- 前端 OCR 上传与简单可视化
- API 访问令牌校验
